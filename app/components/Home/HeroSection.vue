<template>
  <section class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
    <div class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
      <div>
        <div class="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700">
        ...
        <h1 class="max-w-3xl text-3xl font-black tracking-normal text-slate-950 sm:text-4xl md:text-5xl">
          {{ $t('hero.title.prefix') }}
          <span class="block text-emerald-600">{{ $t('hero.title.suffix') }}</span>
        </h1>
      ...
      <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div class="mt-5 h-3 overflow-hidden rounded-full bg-white">
          <div class="h-full rounded-full bg-emerald-500 transition-all duration-700" :style="{ width: `${stats.percentage}%` }"></div>
        </div>
